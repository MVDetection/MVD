...
def translatePDGByNode(db, func_node):
    ...
    for i in range(len(Edges)):
        print[Edges[i].start_node._id, Edges[i].end_node._id], Edges[i].start_node['code'], Edges[i].end_node['code']
    graph_type = 'pdg'
...
def get_all_calls_node(db, testID):
    ...
    print
    "list_all_funcID", list_all_funcID
    print
    "lenth", len(list_all_funcID)
    ...
    if list_all_callee_node == []:
        ...
        return [(str(node._id), node.properties['code'], str(node.properties['functionId'])) for node in
                list_all_callee_node]
...
def getClassByObjectAndFuncID(db, objectname, func_id):
    ...
    for cfg_node in all_cfg_node:
        if cfg_node.properties['code'] == objectname and cfg_node.properties['type'] == 'Statement':
            print
            objectname, func_id, cfg_node.properties['code'], cfg_node._id
            ...
            for node in results_2:
                print
                node.properties['location'].split(':')[0], location_row
...
def getCallGraph(db, testID):
    ...
    for func_node in list_all_func_node:
        # print(func_node)
        prop = {'funcname': func_node.properties['name'], 'type': func_node.properties['type'],
                'filepath': func_node.properties['filepath']}
        ...
        if function_name.find('::') != -1:  # if is a function in class, have two problems
            ...
            if func_name == classname:  # is a class::class, is a statementnode or a iddeclnode
                ...
                for node in list_delete_node:
                    ...
                    for cfg_node in all_cfg_node:
                        if cfg_node.properties['location'] != None and cfg_node.properties['location'].split(':')[
                            0] == delete_loc and cfg_node.properties['code'] != 'delete' and cfg_node.properties[
                            'code'] != '[' and cfg_node.properties['code'] != '[':
                            ...
                            if ob_classname == classname:
                                for p_n in pdg.vs:
                                    ...
                                    if p_n['name'] == str(node._id):
                                        ...
                                        for edge in pdg.es:
                                            if pdg.vs[edge.tuple[0]] in list_s and pdg.vs[edge.tuple[1]] == p_n and \
                                                    edge['var'] == objectname:
                                                # print (functionID, str(pdg.vs[edge.tuple[0]]['name']))
                                                list_callee_id.append(
                                                    (str(functionID), str(pdg.vs[edge.tuple[0]]['name'])))
                                            else:
                                                continue
                            ...
                            if ob_classname == classname:
                                ...
                                for p_n in pdg.vs:
                                    if p_n['name'] == str(node._id):
                                        ...
                                        list_callee_id.append(
                                            (functionID, str(cfg_node._id)))  # delete and its object node
